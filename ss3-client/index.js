  ...
  return request
    .post({
      ...
      headers: {
        ...
        Authorization:
          "Basic NGRmNTU2MjctNDZiMi00ZTJjLTg2NmItMTUyMWIzOTVkZWQyLjEtMC0wLldlYkFwcC5zaW1wbGlzYWZlLmNvbTo="
      },
      body: {
        grant_type: "password",
        device_id: "WebApp",
    ...
    .then(
      function(parsedBody) {
        ...
        expireDate.setSeconds(
          expireDate.getSeconds() + Math.round(thisObj.expires_in * 0.9)
        );
...
SS3Client.prototype.logout = function() {
  ...
  return this.authCheck().then(
    ...
    function() {
      log(
        "SS3Client no need to delete token as it is invalid: " + thisObj.token
      );
    }
  );
};
    ...
    .then(
      function() {
        if (thisObj.isExpired()) {
          log(
            "SS3Client Auth check passed, but token is about to expire so acquiring new token. old token: " +
              thisObj.token
          );
      ...
      function() {
        log(
          "SS3Client Auth check failed so acquiring new token. old token: " +
            thisObj.token
        );
        return thisObj.initToken();
      }
...
SS3Client.prototype.invokeSSGet = function(reqOptions) {
  var thisObj = this;
  return this.initTokenIfNeeded().then(function() {
    reqOptions.headers = {
      "Content-Type": "application/json; charset=utf-8",
...
SS3Client.prototype.initUserId = function() {
  var thisObj = this;
  return this.getUserId().then(function(parsedBody) {
...
SS3Client.prototype.initSubId = function() {
  ...
  var reqOptions = {
    url:
      "https://api.simplisafe.com/v1/users/" +
      thisObj.userId +
      "/subscriptions?activeOnly=false",
    json: true,
    jar: true
  };
  return this.invokeSSGet(reqOptions).then(function(parsedBody) {
...
SS3Client.prototype.getAlarmState = function() {
  ...
  var self = this;
  return this.getAlarmStateNoRetry().then(function(alarmState) {
    if (alarmState === "error") {
      log(
        "SS3Client got alarm state: " +
          alarmState +
          " so retrying getAlarmState in " +
          self.retryInSec +
          " seconds"
      );
      ...
      function retryGetAlarmState(retriesLeft) {
        ...
        getAlarmStateFunc().then(function(alState) {
...
SS3Client.prototype.getAlarmStateNoRetry = function() {
  return this.getSub(this.subId).then(function(parsedBody) {
